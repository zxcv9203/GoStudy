...
- 매개변수가 go의 스택에 저장될 때 명명되어, 어셈블리어 코드를 인식할때 더 용이하다.
([추가설명](./함수스택.md))
...
익명함수는 단어에서도 알 수 있듯이 '이름이 없는 함수'이다. 본래 함수의 이름은 해당 함수만이 갖는 고유한 시그니처 중의 하나로, 특히, 가독성의 측면에서 '해당 함수가 왜 필요하고 어떠한 기능을 하는지'를 대략적으로 표현하여 코드 가독성에 큰 역할을 하게 된다. 그렇다면 왜 굳이 이름이 없는 함수를 작성하여 가독성이 떨어지는 코드를 짤 수 있게 한 것인지 의문이 발생하는데...
...
defer는 특정 함수나 메소드를 함수 가장 마지막에 실행하도록 하는 제어 구문이다.
...
defer: 5
```
결과에서 볼 수 있듯 defer는 argument는 키워드가 실행될 때를 기준으로 평가한다는 특징을 갖는다. 그래서 defer 키워드가 적용된 함수는 맨 마지막에 실행이 되더라도 이전의 argument 값을 기반으로 실행이 된다.
...
defer를 사용하면 각 호출은 스택에 저장된다. 최종적으로는 LIFO(후입선출) 방식으로 각 호출이 처리되어, 위의 프로그램에서는 역방향으로 문자를 출력하는 결과가 나오게 된다.
...
defer가 사용되는 대표적인 사용처는 리소스를 관리하는 부분이다.

아례 예시에서는 defer가 사용되지 않은 경우, 파일을 close하는 예시이다.
```go
package main
...
이에 대해서는 defer를 사용하지 않더라도 io.WriteString의 에러를 처리하는 구문에 Close 함수를 추가하여 해결할 수 있다.
```go
package main
...
다만, close 함수 자체가 문제가 생기는 경우 이를 잡아낼 수가 없다. 이를 해결하기 위해서, return close()를 추가하여, 에러를 리턴하는 방식으로 close에서 발생한 에러를 프로그램에 알려줄 수 있다.

close가 두번 호출될 가능성이 존재하지만(return -> defer), 무엇이 잘못되는지 넘겨줄 수 있다는 점에서 고려해볼만 한듯 싶다.
```go
package main
